...
(function(){
	...
	*/
	...
	function renderMode7(toDraw){
		...
		var x1 = Origami.world.mode7.canvas.width/2;
		var y1 = Origami.world.mode7.canvas.height;
		var x2 = -Origami.cam.x;
		var y2 = -Origami.cam.z;
		var c = Math.cos(Math.PI + Origami.cam.ry);
		var s = Math.sin(Math.PI + Origami.cam.ry);
		var m = {
			a: c,
			b: s, 
			c: -s, 
		...
		Origami.world.mode7.ctx.transform(m.a, m.b, m.c, m.d, m.x, m.y);
		//Origami.world.mode7.ctx.translate(-Origami.cam.x, -Origami.cam.z);
		...
		for(var y = Origami.size.h; y > Origami.size.hh; y-= Origami.world.mode7.step){
			...
			if(Origami.world.mode7.canvas.width/2-x>=0 && z>0 && Origami.world.mode7.canvas.height - z>0 && Origami.world.mode7.canvas.width/2+x<Origami.world.mode7.canvas.width){
				toDraw.push({
					type: "gfx",
					zpos: Infinity,
					gfx: Origami.world.mode7.canvas,
					sx: Math.round(Origami.world.mode7.canvas.width/2 - x),
					sy: Math.round(Origami.world.mode7.canvas.height - z),
					sw: Math.round(2*x),
				...
				//if(Origami.size.h - y > 10*Origami.world.mode7.step) break;
	...
	*/
	...
	Origami.draw = function(toDraw){
		for(i=0; i<toDraw.length; i++){
			shape = toDraw[i];
			if(shape.type == "transform"){
				Origami.ctx.save();
				Origami.ctx.transform(shape.matrix.a, shape.matrix.b, shape.matrix.c, shape.matrix.d, shape.matrix.x, shape.matrix.y);
				//Origami.ctx.translate(10, 0);
				Origami.draw(shape.toDraw);
				Origami.ctx.restore();
			}else if(shape.type == "3DSprite"){
